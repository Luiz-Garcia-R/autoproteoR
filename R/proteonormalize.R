...
#' @param method Character. Normalization method. Options: \code{"none"} (default), \code{"log2"}, \code{"log10"}, \code{"quantile"}, \code{"median"}, \code{"zscore"} .
...
proteo.normalize <- function(raw_data, metadata, method = c("none", "log2", "log10", "quantile", "median", "zscore"),
...
")
  ...
  if (nrow(expr_data) == 0) {
    ...
    if (method == "log10") {
      norm_matrix <- log10(norm_matrix + pseudo_count)
    }
    ...
    if (method == "quantile") {
      ...
      norm_matrix <- preprocessCore::normalize.quantiles(norm_matrix)
      checks$quantile <- "Quantile normalization applied"
    }

    if (method == "median") {
      scaling_factors <- apply(norm_matrix, 2, median, na.rm = TRUE)
      norm_matrix <- sweep(norm_matrix, 2, scaling_factors, "/")
      checks$median <- "Median normalization applied"
    }

    if (method == "zscore") {
      norm_matrix <- apply(norm_matrix, 2, scale)
      checks$zscore <- "Z-score standardization applied"
    }
  ...
  # --- Metadata alignment ---
  metadata <- metadata[match(colnames(norm_matrix), metadata[[col_sample]]), , drop = FALSE]
  if (!all(metadata[[col_sample]] == colnames(norm_matrix))) {
